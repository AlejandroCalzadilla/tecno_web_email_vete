...
/**
 *  clase que representa la tabla de palabras claves.
 *  la funcion estaEnTPC() verifica si un lexema se encuentra en la tabla de palabras claves.
 *  los palabrasClave y tokens se encuentran en listas enlazadas.
 *  se usa para reconocer las palabras reservadas y el token ID.
 *
 *   el tostr que se ingresa a los elementos de los token son como las rutas  , cliente.index algo asi .
 *   coinciden con las palabras clave
 * */



public class TablaDePalabrasClave {
//TPC se usa para reconocer las palabras reservadas y el token ID.

    private static final LinkedList<String> palabrasClave = new LinkedList<>(Arrays.asList(
            "HELP",
            "TRUE",
            "FALSE",
...
//verifica si un lexema se encuentra en la tpc
    public static Token estaEnTPC(String lexema) {
        ...
        for (int i = 0; i < palabrasClave.size(); i++) {
            if (palabrasClave.get(i).toUpperCase().equals(lexema)) {
